...
- Incremental compilation is enabled in `tsconfig.base.json`, and `tsc --build` caches lib output, so each package writes a `.tsbuildinfo` (already gitignored) and repeat typechecks only recheck what changed. `pnpm run typecheck` and the per-package `tsc -p tsconfig.json --noEmit` scripts pick this up automatically — no extra flag is needed.
